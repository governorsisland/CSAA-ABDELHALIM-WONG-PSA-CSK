# Case CGC-25-631801 — Independent Equity Action to Set Aside Judgment

**Court:** Superior Court of California, County of San Francisco
**Case No.:** CGC-25-631801
**Related to:** CGC-21-594102 (underlying trial)
**Plaintiff:** Franciscus Dylan Rosario
**Case Management Conference:** May 27, 2026 at 10:30 a.m., Department 610

[← Back to Case Root](../ROSARIO.md)

---

## Theory of the Case

This is an independent action in equity to set aside the final judgment entered in CGC-21-594102
on the grounds of **extrinsic fraud** that deprived Plaintiff of a fair opportunity to present
material evidence and fairly litigate liability.

**Authority:** Code Civ. Proc. § 187; inherent equitable power; *Kulchar v. Kulchar* (1969) 1 Cal.3d 467.

### The Three-Leg Framework of Extrinsic Fraud

1. **Defense-side knowledge and access** — Defendants had possession of and access to the
   critical evidence materials (911 call, body camera footage, CAD logs) through prior
   receipt and production history.

2. **False and misleading representations to the Court** — Defense made false and misleading
   representations about receipt and prior production, including false denials of knowledge
   and false claims of "unknown provenance."

3. **Court reliance** — The Court relied on those representations to deny continuances,
   exclude materials, and defer admission, creating concrete procedural barriers that
   prevented Plaintiff from fairly presenting the evidence.

### The Critical Evidence

- **The Navaratnasingham "Dolan" admission (RT 7, p. 214, ll. 13-15):** On April 16, 2025, defense
  counsel stated: *"Sure. They were provided by Dolan. He has had them for whatever, sure."* This
  contradicts her MIL No. 17 representations (Jan. 17, 2025) that the materials were "not produced
  during discovery," "eleventh hour," and of "unknown provenance," and her Feb. 18, 2025 hearing
  statements ("news to me," "did not receive until the day of the MSC"). Body cams, 911 audio, and
  CAD log were served together in the same discovery production (Jessup testimony, Hearing Tr. pp. 139-140).
  The Substitution of Attorney (May 4, 2023, Transaction ID 69953800) establishes PSA had the defense
  file for approximately 20 months before trial.
- **911 call recording:** Defendant Abdelhalim stated *"I barely hit him with anything."* Authenticated
  at trial: "Yes. That is my voice." (RT Day 8, l. 1979.) Plaintiff's former counsel obtained this
  through a routine public records request within three months of the collision. CSAA never obtained
  it before issuing its denial.
- **Body-worn camera footage:** Contained eyewitness statements material to liability. Navaratnasingham
  admitted the body cams were "provided by Dolan" and that the defense had curated five videos down to
  8-9 minutes for the jury.
- **CAD dispatch logs:** Contemporaneous record of the incident; part of the same Dolan production.

---

## Defendants

| # | Defendant | Role |
|---|---|---|
| 1 | Subhi Abdelhalim | Judgment beneficiary in underlying action |
| 2 | CSAA Insurance Exchange | Liability insurer; funded, directed, and controlled the defense |
| 3 | Phillips, Spallas & Angstadt LLP | Retained successor defense firm |
| 4 | Priya D. Navaratnasingham | Defense attorney (PSA); made false representations to Court |
| 5 | Alberto Reyna | Defense attorney (PSA); made false representations to Court |
| 6 | Michael R. Chambers | Original defense counsel; transferred file without documentation |
| 7 | Carbone, Smith & Koyama | Original defense firm; transferred file May 4, 2023 |

---

## Filed Documents

| # | Document | PDF |
|---|---|---|
| 1 | Second Amended Complaint | [PDF](01-Second-Amended-Complaint.pdf) |
| 2 | Notice of Related Cases | [PDF](02-Notice-of-Related-Cases.pdf) |
| 3 | Memorandum — Fraud on the Court | [PDF](03-Memorandum-Fraud-on-Court.pdf) |
| 4 | Declaration Re Service | [PDF](04-Declaration-Re-Service.pdf) |
| 5 | Demand — Judicial Summary of Facts | [PDF](05-Demand-Judicial-Summary-of-Facts.pdf) |
| 6 | Requests for Admission | [PDF](06-Requests-for-Admission.pdf) |
| 7 | Ex Parte — Document Preservation | [PDF](07-Ex-Parte-Document-Preservation.pdf) |
| 8 | Request for Judicial Notice | [PDF](08-Request-for-Judicial-Notice.pdf) |
| 9 | Exhibits -- Court Filing Set | [PDF](09-Exhibits-631801-Court.pdf) |
| 10 | Exhibits -- Memorandum and Supplemental | [PDF](10-Exhibits-Memorandum-Supplemental.pdf) |
| 11 | Memorandum -- Navaratnasingham "Dolan" Admission (Fatal Proof of Extrinsic Fraud) | [PDF](11-Motion-Extrinsic-Fraud-Navaratnasingham-Admission.pdf) |

---

## Relief Sought

1. **Equitable vacatur** of the judgment in CGC-21-594102
2. **Retrial** on a fair, complete, and accurate evidentiary record
3. **Ancillary equitable monetary relief** (restitutionary; bounded to the fraud-driven period):
   - Out-of-pocket costs for emergency foundation work and record reconstruction
   - Incremental medical/therapy/pharmacy expenses tied to the fraud period
   - Time and opportunity costs caused by the process corruption

---

## Key Legal Authorities

| Authority | Relevance |
|---|---|
| *Kulchar v. Kulchar* (1969) 1 Cal.3d 467 | Standard for equitable relief to set aside judgment |
| Code Civ. Proc. § 187 | Court's inherent equitable power |
| *In re Marriage of Kieturakis* (2006) 138 Cal.App.4th 56 | Extrinsic fraud elements |
| Evidence Code § 1150 | Jury misconduct inquiry standard |
| *Weaver v. ZeniMax Media* (2004) 116 Cal.App.4th 359 | False representation to court as extrinsic fraud |
